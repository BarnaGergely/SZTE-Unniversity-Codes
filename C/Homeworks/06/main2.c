...
void rendez(LEGO_lap LEGO_lapok[], int n) {
    ...
    while (rendes == 0){
        int javitott = 0;
        for (int i = 0; i < n-1; ++i) {
            if(LEGO_lapok[i].a_oldal*LEGO_lapok[i].b_oldal > LEGO_lapok[i+1].a_oldal*LEGO_lapok[i+1].b_oldal){
                struct LEGO_lap TEMP;
                TEMP = LEGO_lapok[i];

                LEGO_lapok[i] = LEGO_lapok[i+1];

                LEGO_lapok[i+1] = TEMP;
                javitott++;
            }
        }
        if(javitott == 0) rendes = 1;
    }
}
